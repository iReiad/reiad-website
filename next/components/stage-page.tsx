/* ============================================================
   ...
   `/money/<stage>`, `/deutsch/<stufe>`, `/quran/<dhap>` and
   `/english/<term>`. It is the page a reader navigates the school
   by, and the one that says how much of it there is.
   ...
   Its address is `/[section]/[slug]`, and so is an article's, so
   Next can only be given one page for the two. `[section]/[slug]/
   page.tsx` is that page and it dispatches on `isSchool()`: a
   section is insights, cooking or travel, a school is money,
   deutsch, quran or english, and the two can never collide. This
   was a route of its own while the ladder ended in `index.html`.
   ============================================================ */
...
export async function StagePage({ section, slug }: { section: string; slug: string }) {
  const found = await getStage(section, slug);
  if (!found) notFound();
        ...
        <div className="wrap">
          ...
          {school === "money" ? (
            <p className="contents-footlink">
              <a href="/money/contents">সব বিষয় এক নজরে, পুরো সূচিপত্র →</a>
            </p>
          ) : null}
