...
<p>The test above assumes the event window's only unusual news is the one being studied. A dividend declared the same week as the earnings figure, a court ruling landing on the same day as a product launch, or an index rebalancing that happens to fall inside the window will move the abnormal return too, and the test cannot tell the two apart. The fix is not statistical, it is a check of the news wire for every day inside the window, not only the calendar date an announcement was formally dated, before the cumulative abnormal return is credited to the event the study set out to test.</p>

<h2>Clustered event dates</h2>
<p>The t test above treats the residual standard deviation as the whole story of how much a cumulative abnormal return like this one varies by chance. That holds for one firm on one date. It stops holding the moment many firms share the same event date, a national budget, a policy rate decision, a regulatory deadline that a whole sector reports against on the same day, because those firms' abnormal returns then share the same market-wide shock and are no longer independent draws. Averaging or summing across such a cluster as though each firm were a separate, unrelated event overstates how confident the result should be. The fix is to treat the clustered date as one observation, not many: build one cumulative abnormal return for the whole cluster, or adjust the variance in the test for the correlation the shared date introduces, rather than running the simple test above once per firm and counting each as independent evidence.</p>
...
<p>ওপরের পরীক্ষা ধরে নেয়, ইভেন্ট উইন্ডোর একমাত্র অস্বাভাবিক খবর সেটাই যা পরীক্ষা করা হচ্ছে। আয়ের সংখ্যার একই সপ্তাহে ঘোষিত একটা লভ্যাংশ, একটা পণ্য চালুর একই দিনে আসা একটা আদালতের রায়, অথবা উইন্ডোর ভেতরে পড়ে যাওয়া একটা ইনডেক্স পুনর্বিন্যাসও অস্বাভাবিক রিটার্নকে নাড়াবে, আর পরীক্ষাটা দুটোকে আলাদা করতে পারবে না। এর সমাধান পরিসংখ্যানের নয়, উইন্ডোর ভেতরের প্রতিটা দিনের খবরের তার দেখা, শুধু ঘোষণাটার আনুষ্ঠানিক তারিখ নয়, তারপর CAR-টা যে ঘটনা পরীক্ষা করার কথা ছিল তার নামে লেখা।</p>

<h2>একসঙ্গে জমা হওয়া ইভেন্ট তারিখ</h2>
<p>ওপরের t পরীক্ষা রেসিডুয়াল স্ট্যান্ডার্ড ডেভিয়েশনকে এই CAR-এর মতো একটা সংখ্যা কাকতালীয়ভাবে কতটা বদলাতে পারে তার পুরো গল্প ধরে নেয়। একটা কোম্পানি একটা তারিখে হলে সেটা ঠিক থাকে। অনেক কোম্পানি একই ইভেন্ট তারিখ ভাগ করলেই সেটা ভেঙে পড়ে, একটা জাতীয় বাজেট, একটা নীতি হারের সিদ্ধান্ত, একটা খাতের সব কোম্পানি একই দিনে যার বিরুদ্ধে রিপোর্ট করে এমন কোনো নিয়ন্ত্রক সময়সীমা, কারণ তখন সেই কোম্পানিগুলোর অস্বাভাবিক রিটার্ন একই বাজারজোড়া ধাক্কা ভাগ করে নেয়, আর আলাদা আলাদা স্বাধীন টান থাকে না। এমন একটা দলকে যেন প্রতিটা কোম্পানি আলাদা, সম্পর্কহীন ঘটনা, এভাবে গড় করা বা যোগ করা ফলাফলে যতটা আস্থা রাখা উচিত তার চেয়ে বেশি দেখায়। সমাধান হলো একসঙ্গে জমা হওয়া তারিখটাকে একটা পর্যবেক্ষণ হিসেবে ধরা, অনেকগুলো নয়: পুরো দলের জন্য একটাই CAR তৈরি করা, অথবা ভাগ করা তারিখটা যে সহসম্পর্ক আনে তার জন্য পরীক্ষার ভেদাঙ্ক ঠিক করা, প্রতিটা কোম্পানির জন্য আলাদা করে ওপরের সহজ পরীক্ষাটা একবার করে চালিয়ে প্রতিটাকে স্বাধীন প্রমাণ হিসেবে গোনার বদলে।</p>
