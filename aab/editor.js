...
const keptClasses = (node) => CLASS_CARRIERS.has(node.tagName)
...
export function sanitize(html) {
    ...
    const walk = (node) => {
        ...
        if (!KEEP.has(tag) && !structural && RENAME[tag]) {
            ...
            const hasBlock = [...node.children].some((c) => /^(P|H2|H3|UL|OL|BLOCKQUOTE|FIGURE|TABLE|HR)$/.test(c.tagName));
...
export function slugify(s) {
    ...
    for (const w of words) {
        if (slug && (slug + "-" + w).length > 40)
            break;
        slug = slug ? `${slug}-${w}` : w;
    }
    return slug || `article-${new Date().toISOString().slice(0, 10)}`;
}
export function readingStats(html) {
...
export const CAPTION_HINT = "Caption: click to edit";
/** Captions the writer never touched shouldn't ship. */
export function dropUntouchedCaptions(html) {
    const doc = new DOMParser().parseFromString(html, "text/html");
    doc.querySelectorAll("figcaption").forEach((c) => {
        if ((c.textContent ?? "").trim().startsWith(CAPTION_HINT))
            c.remove();
    });
...
const TABLE_SKELETON = '<div class="table-scroll"><table><thead><tr>'
...
export function figureHtml({ url, width, height }, alt = "") {
...
export function createEditor({ root, onChange, lang = () => "en", toast = () => { }, pickPhoto = () => { }, onSave, onPublish, }) {
    ...
    function outerBlock(node) {
        ...
        let found = null;
        while (el && el !== root) {
            if (el instanceof Element && el.matches(BLOCK_SEL))
                found = el;
            el = el.parentElement;
        }
        return found;
    ...
    const exec = (cmd, value = null) => document.execCommand(cmd, false, value ?? undefined);
    ...
    ["dragenter", "dragover"].forEach((ev) => on(root, ev, (e) => {
        if (!e.dataTransfer?.types.includes("Files"))
            return;
        e.preventDefault();
        root.classList.add("drop-target");
    }));
    ["dragleave", "drop"].forEach((ev) => on(root, ev, () => root.classList.remove("drop-target")));
    on(root, "drop", (e) => {
    ...
    function drawSlash(query) {
        ...
        slashMenu.replaceChildren(...slashShown.map((item, i) => {
            ...
            row.append(Object.assign(document.createElement("span"), { textContent: item.label }), Object.assign(document.createElement("span"), { className: "mono", textContent: item.hint }));
    ...
    function setFromSet(figure, set, wanted) {
        set.forEach(([cls]) => { if (cls)
            figure.classList.remove(cls); });
        if (wanted)
            figure.classList.add(wanted);
    }
    const chosenFrom = (figure, set) => set.find(([cls]) => cls && figure.classList.contains(cls))?.[0] ?? "";
    function showFigBar(img) {
        ...
        const group = (name, set) => {
            ...
            row.append(Object.assign(document.createElement("span"), { className: "mono", textContent: name }), ...set.map(([cls, label, hint]) => chip(label, cls === chosen, () => {
    ...
    on(window, "scroll", () => { if (activeFigure)
        showFigBar(activeFigure); }, { passive: true });
    /* ---------- keyboard ---------- */
    on(root, "keydown", (e) => {
