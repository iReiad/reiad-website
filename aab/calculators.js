...
export const emi = {
    ...
    run(v) {
        ...
        const instalment = (p, r, m) => r > 0 ? (p * r * (1 + r) ** m) / ((1 + r) ** m - 1) : p / m;
...
export const calculatorFor = (id) => CALCULATORS.find((c) => c.id === id);
/** The defaults a calculator opens with, as a plain record. */
export const defaultsFor = (c) => Object.fromEntries(c.fields.map((f) => [f.name, f.value]));
