...
const signClass = (n) => (n > 0 ? "live-up" : n < 0 ? "live-down" : "");
...
function weightRow(h, largest) {
  const width = largest > 0 ? Math.max(2, (h.weightPct / largest) * 100) : 0;
...
async function drawPublic() {
  ...
  $("live-public-return").textContent = PCT(p.returnPct);
  $("live-public-return").className = `v ${signClass(p.returnPct)}`;
  $("live-public-invested").textContent = PCT(p.investedPct, { signed: false });
  $("live-public-cash").textContent = PCT(p.cashPct, { signed: false });
  $("live-public-count").textContent = String(p.count);
  ...
  return el("div", { className: "live-table" },
    el("table", {},
      ...
      el("tbody", {}, rows.map((p) => {
        ...
        const gainPct = w.totalCost > 0 ? (w.unrealizedProfitLoss / w.totalCost) * 100 : 0;
        const weight = invested > 0 ? (w.currentValue / invested) * 100 : 0;
        const width = largest > 0 ? Math.max(2, (w.currentValue / largest) * 100) : 0;
        return el("tr", {},
          el("td", {},
            el("span", { className: "live-name" }, p.instrument?.name ?? "–"),
            el("span", { className: "mono live-ticker" },
              String(p.instrument?.ticker ?? "").split("_")[0])),
          el("td", { className: "mono" }, QTY(p.quantity)),
          el("td", { className: "mono" }, MONEY(p.averagePricePaid, p.instrument?.currency)),
          el("td", { className: "mono" }, MONEY(p.currentPrice, p.instrument?.currency)),
          el("td", { className: "mono" }, MONEY(w.currentValue, currency)),
          el("td", { className: `mono ${signClass(w.unrealizedProfitLoss)}` },
            `${MONEY(w.unrealizedProfitLoss, currency)} (${PCT(gainPct)})`),
          el("td", {},
            el("span", { className: "live-bar" },
              el("span", { className: "live-fill", style: `width:${width}%` })),
            el("span", { className: "mono live-weight" }, PCT(weight, { signed: false }))));
      }))));
}

/* Dividends, summed by month, drawn as one row of columns. One
   hue, because one series; the numbers ride above the columns
   for the months that have room and in the tooltip for all. */
function dividendChart(items, currency) {
  ...
  return el("div", { className: "live-block" },
    ...
    el("div", { className: "live-cols", role: "img",
      ...
      months.map((m, i) => el("div", {
        ...
        el("span", {
          ...
          style: `height:${Math.max(values[i] > 0 ? 4 : 1, (values[i] / top) * 100)}%`,
        }),
    ...
    moves.length
      ? el("ul", { className: "live-list" }, moves.map((t) => el("li", {},
          el("span", {}, `${String(t.type ?? "").toLowerCase().replaceAll("_", " ")} `,
            el("strong", { className: signClass(t.amount) }, MONEY(t.amount, t.currency ?? currency))),
...
function accountDashboard(account, { title, note } = {}) {
  ...
  const cash = s.cash ?? {};
  const gainPct = inv.totalCost > 0 ? (inv.unrealizedProfitLoss / inv.totalCost) * 100 : 0;
  ...
  root.append(el("div", { className: "stat-row" },
    statTile("Account value", MONEY(s.totalValue, currency), `all in, ${currency}`, ""),
    statTile("Invested", MONEY(inv.currentValue, currency), `cost ${MONEY(inv.totalCost, currency)}`),
    statTile("Unrealised", `${MONEY(inv.unrealizedProfitLoss, currency)}`,
      PCT(gainPct) + " on cost", signClass(inv.unrealizedProfitLoss)),
    statTile("Realised, all time", MONEY(inv.realizedProfitLoss, currency), "",
      signClass(inv.realizedProfitLoss)),
    statTile("Free cash", MONEY(cash.availableToTrade, currency),
      cash.inPies ? `plus ${MONEY(cash.inPies, currency)} parked in pies` : "")));

  const positions = account.positions ?? [];
  if (positions.length) {
    root.append(el("div", { className: "live-block" },
      el("h3", {}, `Holdings (${positions.length})`),
      holdingsTable(positions, currency, inv.currentValue ?? 0)));
  } else {
...
let ownKeyMode = null; /* "saved" | {key, env} for the tab */
...
async function drawOwn() {
  ...
  const dash = accountDashboard(live.account, {
    title: ownKeyMode === "saved" ? "Your account, from the saved key" : "Your account, key held by this tab",
  });

  const bar = el("div", { className: "live-actions" },
    el("button", { className: "btn btn-ghost", type: "button", onclick: () => drawOwn() }, "Refresh"),
    el("button", {
  ...
  out.replaceChildren(dash, bar,
    el("p", { className: "muted live-soft" }, "Loading history…"));
...
async function drawAdmin() {
  ...
  /* -- the site key -- */
  const keyInput = el("input", { type: "password", placeholder: "site Trading 212 key", autocomplete: "off" });
  const envPick = el("select", {},
    el("option", { value: "live" }, "Real money"),
    el("option", { value: "demo" }, "Practice (paper)"));
  ...
  keyForm.addEventListener("submit", async (e) => {
    ...
    keyNote.textContent = set?.ok
      ...
      : why(set);
    if (set?.ok) { keyInput.value = ""; drawPublic(); }
  });
  ...
  const maxBox = el("input", { type: "number", min: 1, max: 100, value: view.max });
  const viewNote = el("span", { className: "mono live-form-note" });

  const viewForm = el("form", { className: "live-admin-form" },
    el("label", { className: "live-remember" }, holdBox, el("span", {}, "Show the holdings list at all")),
    el("label", { className: "live-remember" }, nameBox, el("span", {}, "Name the holdings, or number them by size")),
    el("label", { className: "live-remember" }, retBox, el("span", {}, "Show each holding's own return")),
