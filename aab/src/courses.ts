...
/* What the API answers with: the shape of `forBrowser()` in
   `shared/courses.ts`, written out because a browser module
   cannot import the Worker's package. `scripts/check-courses.ts`
   compares these field names against what that function emits. */
...
function markRead(id: string) {
  const set = readSet();
  if (set.has(id)) return;                     // no event for no change
  set.add(id);
  writeJSON(READ_KEY, [...set]);
  dispatchEvent(new CustomEvent(CHANGED));
}

function toggleRead(id: string): boolean {
  const set = readSet();
...
/* ============================================================
   ...
   Flattened once per page and passed around, because the sidebar,
   the percentage bars and the continue button all walk the same
   list and computing it three times is three chances to compute
   it differently.
   ============================================================ */

interface Rung extends Lesson {
  module: string;
  moduleTitle: string;
  id: string;
  url: string;
...
function nextUp(rungs: Rung[], read: Set<string>): Rung | null {
  const mark = readJSON<Bookmark | null>(LAST_KEY, null);
...
async function api<T>(path: string): Promise<Answer<T>> {
  ...
  try {
    ...
    return { ok: true, status: res.status, data: body as T, message: "" };
  } catch {
...
/* ============================================================
   ...
   Plain DOM rather than a template string, because most of what
   is built below carries a listener or a Drive id, and building
   markup by concatenation is how an id ends up somewhere it is
   escaped wrongly. `el()` keeps it short.
   ============================================================ */
...
function meter(done: number, total: number, label?: string): HTMLElement {
...
function drawLesson(
  ...
  if (lesson.transcript) extras.push(fileRow("Transcript", "txt", lesson.transcript));
  for (const file of lesson.files) extras.push(fileRow(file.name, file.ext, file.drive));
  if (extras.length) {
...
function fileRow(name: string, ext: string, drive: string): HTMLElement {
...
function note(root: HTMLElement, title: string, body: string, action?: HTMLElement) {
...
/** 401 and 403 are different sentences, and a page that gives one
    ...
    already signed in. */
function refuse(root: HTMLElement, answer: Answer<unknown>) {
  if (answer.status === 403) {
    note(root, "Not yours",
      "This section is one person's own copy of a third-party course. It is not published.");
    return;
  }
