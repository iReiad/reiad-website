...
import { libraryRow, keepPage, type LibraryRow } from "/saved.js";

/* ============================================================
   Which page this is

   A piece carries `article.article[data-slug]`; a lesson carries
   `article[data-school]` and one of three id attributes. Both are
   identified here by their URL, and that is deliberate rather
   than lazy: the library is about PAGES, one row per address, and
   a slug and a lesson id are two vocabularies that would both
   need mapping back to something a reading list can link to.

   The address is `location.pathname`, with no query and no
   fragment, so the same lesson reached from a hub and from a
   search does not become two rows.
   ============================================================ */

/** What this page is, as the library stores it. */
interface Subject {
...
const icon = (name: keyof typeof ICON): SVGSVGElement => {
...
export function initKeep(root: ParentNode = document): HTMLElement | null {
  ...
  /* Once per page. The module runs this on import and the
     ...
     second time. */
  if (document.querySelector(".keep-bar")) return null;
  ...
  const bar = el("div", { className: "keep-bar", hidden: true },
    keepBtn, noteBtn, say);
  ...
  const panel = el("div", { className: "keep-panel", hidden: true },
    field,
    el("div", { className: "keep-panel-actions" }, saveNote, noteSaid));
  ...
  const write = async (
    ...
    try {
      ...
      /* The trigger in the migration deletes a row that has
         ...
         honest. */
      if (patch.saved === false && !(row?.note)) row = null;
      paint();
      tell.textContent = note;
      setTimeout(() => { tell.textContent = ""; }, 2600);
    } catch (err) {
      tell.textContent = (err as Error).message || "That did not save.";
    }
  };
  ...
  libraryRow(url).then((found: LibraryRow | null) => {
    row = found;
    if (found?.note) field.value = found.note;
    paint();
    bar.hidden = false;
  }).catch(() => {
