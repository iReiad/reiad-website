...
/* What the account holds, and the school to tell when one moves.

     set      a list of ids, reconciled by the subtraction above.
     mark     a bookmark, { id, url, ts }. The newer of the two.
     count    a number that only goes up, unless this device went
              backwards, which is a reset and is respected.
     merge    a map, reconciled entry by entry on each entry's ts.

   THE NAMES ARE STORAGE KEYS AND NONE MAY BE RENAMED: they are
   what is in real browsers and in real accounts, and renaming one
   loses somebody's ticks rather than moving them. `learn-read` is
   the money school's even though the school moved to /money/.
   The union below is what stops a key being added with a rule
   nobody implemented and falling through to the bookmark branch. */
