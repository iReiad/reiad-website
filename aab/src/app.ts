...
import { searchIndex, SEARCH_GROUPS, COUNTS, type SearchEntry } from "/content.js";
...
let INDEX: SearchEntry[] = searchIndex();

export function addToSearchIndex(pieces: Piece[]): void {
...
function initPalette(): void {
  ...
  const count = dialog.querySelector<HTMLElement>("#palette-count")!;
  let active = 0;
  ...
  const render = (query: string) => {
    ...
    const groups = SEARCH_GROUPS
      .map(([kind, label]): [string, SearchEntry[]] =>
        [label, scored.filter((r) => r.item.kind === kind)
  ...
  const move = (delta: number) => {
    const items = list.querySelectorAll<HTMLElement>("li:not(.palette-empty):not(.palette-group)");
  ...
  dialog.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Enter") {
      e.preventDefault();
      list.querySelector<HTMLAnchorElement>("li.active a")?.click();
    }
  });
...
const el = <K extends keyof HTMLElementTagNameMap>(
  tag: K, props: Partial<HTMLElementTagNameMap[K]> = {}, ...kids: Kid[]
...
const toBangla = (n: number) => String(n).replace(/\d/g, (d) => BN_DIGITS[Number(d)]);
...
const COUNT_VALUES: Record<string, number | undefined> = COUNTS;
...
/** The toast keeps its dismissal timer on itself, so a second
    message replaces the first rather than being hidden 2.6s after
    the first one appeared. */
interface Toast extends HTMLElement {
...
export function toast(message: string): void {
  let el = document.getElementById("app-toast") as Toast | null;
  if (!el) {
...
export function download(
  filename: string, data: Blob | string, type = "text/html;charset=utf-8",
): void {
