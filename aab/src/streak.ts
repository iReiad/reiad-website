...
const read = (): string[] => {
  try {
    ...
    return list.filter((d): d is string => typeof d === "string");
  } catch {
    return [];
  }
};

const write = (days: readonly string[]): boolean => {
  try {
...
export const activeDays = (): string[] => [...new Set(read())].sort();

/** Today, recorded once. Returns whether it was new. */
export function markToday(): boolean {
  const days = read();
  const now = today();
 ...
 */
export function daysIn(span: number = 7): number {
