...
export function listThreads(state?: ThreadState): Promise<Thread[]> {
  if (!current()) return Promise.resolve([]);
  const where = state ? `&state=eq.${encodeURIComponent(state)}` : "";
  return get<Thread[]>(
    `threads?select=${THREAD_FIELDS}${where}&order=updated_at.desc`, []);
}

/** Start one. Only the question, because everything else is what
    the thread becomes. */
export async function addThread(question: string): Promise<Thread | null> {
  const row = { question: String(question).slice(0, 200) };
  const back = await send(
    `threads?select=${THREAD_FIELDS}`, "POST", [row], "return=representation",
  ) as Thread[] | null;
  return back?.[0] ?? null;
}
...
export async function saveThread(
  ...
  if (patch.question !== undefined) row.question = String(patch.question).slice(0, 200);
  ...
  const back = await send(
    `threads?id=eq.${encodeURIComponent(id)}&${mine()}&select=${THREAD_FIELDS}`,
    "PATCH", row, "return=representation",
  ) as Thread[] | null;
  return back?.[0] ?? null;
}

export function removeThread(id: string): Promise<boolean> {
  return send(
    `threads?id=eq.${encodeURIComponent(id)}&${mine()}`,
    "DELETE", undefined, "return=minimal",
  ).then(() => true);
}
