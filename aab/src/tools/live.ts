...
interface PublicHolding {
  name?: string; ticker?: string; weightPct?: number; returnPct?: number | null;
}
interface PublicPortfolio {
  at?: string; count?: number; investedPct?: number; cashPct?: number;
  returnPct?: number; holdings?: PublicHolding[] | null;
}
    ...
    el("table", {},
      ...
      el("tbody", {}, rows.map((p) => {
        ...
        return el("tr", {},
          ...
          el("td", { className: "mono" }, MONEY(value, currency)),
          el("td", { className: `mono ${signClass(gain)}` },
            `${MONEY(gain, currency)} (${PCT(gainPct)})`),
          el("td", {},
            el("span", { className: "live-bar" },
              el("span", { className: "live-fill", style: `width:${width}%` })),
...
function accountDashboard(account: Account, { title, note }: {
  ...
  root.append(el("div", { className: "stat-row" },
    ...
    statTile("Invested", MONEY(inv.currentValue, currency), `cost ${MONEY(cost, currency)}`),
