...
import {
  ...
  getProfile, cachedProfile, type Reader,
} from "/account.js";
...
const el = <K extends keyof HTMLElementTagNameMap>(
  ...
  node.append(...kids.filter((k): k is Node | string => Boolean(k)));
  return node;
};
...
function place(menu: HTMLElement, button: HTMLElement): void {
  ...
  menu.style.setProperty("--menu-right",
    `${Math.round(Math.max(8, window.innerWidth - box.right))}px`);
}
...
function say(text?: string | null, state?: string): void {
  note.textContent = text ?? "";
  if (state) note.dataset.state = state;
  else delete note.dataset.state;
}

/* Every destination the account has, in the order somebody would
   look for them. All six are one page: the account page is
   sections, and a menu item is a link to one of them rather than
   six routes that would each have to load the same four
   curricula to draw the same header. */
...
const item = ({ href, icon: name, label, note: sub }: Destination): HTMLAnchorElement =>
  el("a", { className: "acc-item", href, role: "menuitem" },
    el("span", { className: "acc-ico" }, icon(name)),
    el("span", { className: "acc-text" },
      el("strong", { textContent: label }),
      el("small", { textContent: sub })));

/** Signed in: who, then where to. */
function accountFace(user: Reader): HTMLElement {
  const out = el("button", {
    className: "acc-item acc-out", type: "button", role: "menuitem",
  }, el("span", { className: "acc-ico" }, icon("out")),
     el("span", { className: "acc-text" },
       el("strong", { textContent: "Sign out" }),
       el("small", { textContent: "on this device only" })));
  ...
  return el("div", { className: "acc-body" },
    ...
    el("div", { className: "acc-list", role: "menu" },
      ...DESTINATIONS.map(item),
      el("span", { className: "acc-rule", role: "separator" }),
      out),
    note);
}

/** Signed out: why, then two ways in. */
function signInFace(): HTMLElement {
  ...
  const form = el("form", { className: "signin-form", noValidate: true },
    ...
    email,
    el("button", { className: "btn btn-solid", type: "submit",
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      say((err as Error).message || "Could not send that. Try again in a minute.", "warn");
    } finally {
  ...
  return el("div", { className: "acc-body acc-body-in" },
    el("span", { className: "signin-kicker mono", textContent: "Reiad's Library" }),
    el("h2", { textContent: "Sign in" }),
    el("p", { className: "signin-why" },
      "So your place in a course, what you have kept and what you have "
      + "written follow you from a phone to a laptop. Everything on this "
      + "site is readable without an account, and stays that way."),
    el("button", {
...
const reposition = () => { if (menu && anchor) place(menu, anchor); };
...
function close(): void {
  ...
  if (menu?.isConnected) menu.hidePopover();
}
...
function open(button: HTMLElement): void {
  ...
  menu.querySelector<HTMLElement>("a, button, input")?.focus({ preventScroll: true });
}
...
document.addEventListener("click", (e) => {
  if (menu && (e.target as Element | null)?.closest?.(".acc-item[href]")) close();
});
...
export async function initSignIn(): Promise<void> {
