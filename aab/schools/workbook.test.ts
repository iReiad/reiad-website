...
/* The BODY rather than the page, and that is not a detail: the
   ...
   CI, which is exactly the mistake `hub.test.ts` warns about two
   comments up. */
...
/** The two exports that bundle carries. It is imported from a
    ...
    is the file's own claim about it. */
interface BookBundle {
  WorkbookBody: (props: { section: string; slug: string }) => unknown;
  renderToStaticMarkup: (node: unknown) => string;
}

const mod: BookBundle = await import(
  `data:text/javascript;base64,${Buffer.from(bundled.outputFiles[0].text).toString("base64")}`);

/** One practice book, and the storage key it must write. */
interface Book {
  school: string;
  slug: string;
  script: string;
...
/** An element this file has just asserted the markup carries. A
    missing one is a broken harness rather than a failing check,
    so it throws saying which. */
...
for (const book of BOOKS) {
  ...
  Object.assign(globalThis, {
    ...
    setTimeout: (f: () => void, _ms?: number) => { timers.push(f); return timers.length; },
    clearTimeout: () => {},
  });
  ...
  if (Select && !Object.getOwnPropertyDescriptor(Select.prototype, "value")?.set) {
    Object.defineProperty(Select.prototype, "value", {
      ...
      set(this: HTMLSelectElement, v: string) {
  ...
  const nav = need(document.querySelector<HTMLElement>("[data-tag-nav]"), "[data-tag-nav]");
  ...
  const area = need(document.querySelector<HTMLTextAreaElement>("textarea[data-schrift]"),
    "textarea[data-schrift]");
  area.value = "eine Katze";
  area.dispatchEvent(new window.Event("input", { bubbles: true }));
  timers.splice(0).forEach((f) => f());     // the debounce

  const written: Record<string, string> = JSON.parse(store.get(book.writeKey) || "{}");
  ...
  if (reveal) {
    const card = need(reveal.closest(".buch-tag"), ".buch-tag around [data-antwort]");
  ...
  const tick = document.querySelector<HTMLElement>("[data-fertig]");
