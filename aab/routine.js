 ...
 */
export function todayFor(roll = 4, now = new Date()) {
...
export function daysAround(day, back, forward = 0) {
    const out = [];
...
export function listTemplates() {
    return get("routine_templates?select=id,owner_id,name,description,data&order=owner_id.asc,name.asc", []);
}
...
export async function activeRoutine() {
    const rows = await get(`routines?select=${ROUTINE_FIELDS}&is_active=is.true&order=created_at.asc&limit=1`, []);
    return rows[0] ?? null;
}
/**
 ...
 */
export async function makeRoutine(name, shape) {
    const who = current();
...
export async function saveRoutine(id, patch) {
    ...
    if (!res.ok)
        throw new Error(`That did not save (${res.status}).`);
    return true;
}
/* ============================================================
   The days
...
/** One day, or null if nothing has been marked on it yet.
    ...
    it. */
export async function dayEntry(date) {
    const rows = await get(`routine_entries?select=${ENTRY_FIELDS}&entry_date=eq.${encodeURIComponent(date)}&limit=1`, []);
    return rows[0] ?? null;
}
...
export function daysBetween(from, to) {
    return get(`routine_entries?select=${ENTRY_FIELDS}`
 ...
 */
export async function saveDay(routineId, date, day) {
    const who = current();
    const head = await headers({
    ...
    for (const [id, m] of Object.entries(day.marks)) {
        if (typeof m === "number" && m > 0)
            marks[id] = Math.min(1, m);
    }
    const res = await fetch(`${REST}/routine_entries?on_conflict=user_id,entry_date&select=${ENTRY_FIELDS}`, {
        method: "POST",
        headers: head,
        body: JSON.stringify([{
                user_id: who.id,
 ...
 */
export async function everMarked(taskId) {
    ...
    if (!head)
        return 0;
    try {
        const res = await fetch(`${REST}/routine_entries?select=id&marks->>${encodeURIComponent(taskId)}=not.is.null`, { method: "HEAD", headers: head });
        if (!res.ok)
            throw new Error(String(res.status));
...
export function everyNote() {
    return get(`routine_entries?select=entry_date,note,mood&note=not.is.null&note=neq.`
