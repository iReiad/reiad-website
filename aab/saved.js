...
/* ============================================================
   Threads: a question somebody is chasing

   The fifth thing an account holds, and the first that is a place
   to WORK rather than a record of something that happened. The
   table's own header says why it is one table and one jsonb; what
   matters here is that a save is one round trip, because a
   working surface that can half-save is a working surface nobody
   trusts.
   ============================================================ */
const THREAD_FIELDS = "id,question,state,tags,body,created_at,updated_at";
...
export function listThreads(state) {
    if (!current())
        return Promise.resolve([]);
    const where = state ? `&state=eq.${encodeURIComponent(state)}` : "";
    return get(`threads?select=${THREAD_FIELDS}${where}&order=updated_at.desc`, []);
}
/** Start one. Only the question, because everything else is what
    the thread becomes. */
export async function addThread(question) {
    const row = { question: String(question).slice(0, 200) };
    const back = await send(`threads?select=${THREAD_FIELDS}`, "POST", [row], "return=representation");
    return back?.[0] ?? null;
}
/** Change one, and only the fields named.
    ...
    a second device. */
export async function saveThread(id, patch) {
    const row = {};
    if (patch.question !== undefined)
        row.question = String(patch.question).slice(0, 200);
    if (patch.state !== undefined)
        row.state = patch.state;
    if (patch.tags !== undefined) {
        /* Lowercased and deduplicated here rather than in the page,
           so that `Banks` and `banks` are one tag however many places
           learn to write one. */
        row.tags = [...new Set(patch.tags
    ...
    if (!Object.keys(row).length)
        return null;
    const back = await send(`threads?id=eq.${encodeURIComponent(id)}&${mine()}&select=${THREAD_FIELDS}`, "PATCH", row, "return=representation");
    return back?.[0] ?? null;
}
export function removeThread(id) {
    return send(`threads?id=eq.${encodeURIComponent(id)}&${mine()}`, "DELETE", undefined, "return=minimal").then(() => true);
}
