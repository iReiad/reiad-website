/* ============================================================
   curriculum.js: the English school, in one file.

   THIS IS THE ONE FILE YOU EDIT to add, rename or reorder
   anything under /english/. Everything else reads from it:

     /english/index.html + hub.js     the ladder, the resume card
     /english/<term>/index.html       a Next route, from the rows
     /english/<term>/<part>.html      a Next route, from the rows
     /english/term-1/workbook.html    a Next route, components/workbook.tsx
   ...
   term    slug     folder name, and the id progress is stored under
           kicker   the tiny label above the name ("টার্ম ১")
           bn       the name in Bangla
           en       the name in English, because this is an English
                    course and the learner should meet the word
           icon     a key in /english/icons.js
           who      who this term is for, in one line
           blurb    what it teaches, two lines at most
           can      what you will be able to DO at the end of it
           minutes  the daily sitting this term asks for, [from, to]
           status   "live" | "soon"
           workbook { slug, days } when the term has a practice
                    book. `days` is declared rather than counted
                    because the browser draws a bar from it and
                    must not download the whole book to do so;
                    build-english.mjs asserts it against the data
           chorcha  what the daily practice is for a term with no
                    book, so the absence reads as a decision

   section id       anchor on the term page
           bn / en  the segment's name in both

   part    slug     file name (no .html)
           n        the part number the deck itself uses. It is on
                    the card and in the eyebrow, because a learner
                    with the deck open should be able to match the
                    two without counting
...
/* ------------------------------------------------------------
   ...
   array above and rerunning build-english.mjs.
   ------------------------------------------------------------ */
