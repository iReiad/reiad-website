...
export const getQuestions = async (slug) => (await api(`questions?slug=${encodeURIComponent(slug)}`))?.questions ?? null;
export const search = async (q) => (await api(`search?q=${encodeURIComponent(q)}`))?.results ?? null;
/* ---------- writing ---------- */
...
export const react = (slug, kind) => api("signals/react", { method: "POST", body: { slug, kind } });
export const reactions = async (slug) => (await api(`signals/react?slug=${encodeURIComponent(slug)}`))?.counts ?? null;
/* ---------- media ----------
...
export const listMedia = (slug = "") => api(`media${slug ? `?slug=${encodeURIComponent(slug)}` : ""}`);
/* ---------- Notion ----------
...
export function countView(path = location.pathname) {
    ...
    whenActivated(() => api("signals/view", { method: "POST", body: { path }, timeout: 3000 }));
}
