...
import type { ReadSection } from "/content.js";

const isBn = (): boolean => document.documentElement.lang === "bn";
/* The German pages are lang="bn" throughout, so a crumb reading
   "60 দিনের খাতা" next to Bangla prose reads as a glitch. */
const bn = (n: number | string): string =>
  String(n).replace(/\d/g, (d) => "০১২৩৪৫৬৭৮৯"[Number(d)]);
const HOME = (): string => (isBn() ? "হোম" : "Home");
const LEARN = (): string => (isBn() ? "শেখার লাইব্রেরি" : "Learn");
const DEUTSCH = (): string => (isBn() ? "জার্মান" : "Deutsch");
const QURAN = (): string => (isBn() ? "কুরআনের আরবি" : "Qur'anic Arabic");
const ENGLISH = (): string => (isBn() ? "মন থেকে ইংরেজি" : "English From The Heart");
const SKILLS = (): string => (isBn() ? "দক্ষতা" : "Skills");
const COURSES = (): string => (isBn() ? "কোর্স" : "Courses");
const sectionName = (section: ReadSection): string =>
...
/** The page's own name, with the site's name taken off the end.
    ...
    than an empty crumb. */
const SUFFIX = new RegExp(`\\s*[,\u00b7\\u2013|-]?\\s*${
  SITE.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*$`);

const stripSite = (title: string): string =>
  title.replace(SUFFIX, "").trim() || title.trim();
...
interface Trail { crumbs: Crumb[]; here: string; }

function trailFor(path: string): Trail {
  ...
  if (p.startsWith("/skills/courses/")) {
    ...
    if (p === "/skills/courses/index.html") return { crumbs, here: COURSES() };
  ...
  if (page) return { crumbs, here: stripSite(page.title) };
...
export function setHere(name: string): void {
  ...
  const ld = document.querySelector<HTMLScriptElement>(
    'script[type="application/ld+json"][data-crumbs]');
  if (!ld) return;
  try {
