...
`next/account.test.ts` is the guard: 160 checks in a real browser
...
node next/account.test.ts        # the account's five features, the popover
                                  ...
                                  # the real CSP (160 checks, needs the Next
...
node next/research-studio.test.ts  # the Research Studio: the capture box
                                   ...
                                   # (72 checks, needs the Next build and a
                                   # browser, skips without)
...
and `foods.ts`, that tool's portion library for Bangladesh and
the UK, where every row is said in both languages, an `id` is a
key already in somebody's log, and every rice, dal and pasta row
