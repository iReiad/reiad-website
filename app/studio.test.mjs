/* ============================================================
   studio.test.mjs: the React Studio, driven in a real browser.

     node app/studio.test.mjs

   OPTIONAL, like the other browser tests: it needs Playwright,
   and nothing about building or deploying the site depends on it.

       npm i -D playwright
       PLAYWRIGHT=/path/to/playwright node app/studio.test.mjs
   ...
   both Studios, and `aab/studio.test.mjs` already drives all 70 of
...
try {
  ...
  console.log("  npm i -D playwright   (or: PLAYWRIGHT=/path/to/playwright node app/studio.test.mjs)");
  process.exit(0);
}
if (!chromium) {
...
async function studio({ backend, fixtures = {} }) {
...

{
  ...
  check("the hint names the mount it will be published at",
    (await page.locator("#section-hint").textContent()).startsWith("/insights/"));
  await page.locator("#f-section-seg .chip", { hasText: "রান্নাঘর" }).click();
  await page.waitForTimeout(300);
  check("choosing another moves the mount",
    (await page.locator("#section-hint").textContent()).startsWith("/cooking/"));
  check("and says so in the bar",
    (await page.locator("#now-line").textContent()).includes("Cooking"));
  ...
  check("its ✕ says what it removes, for a screen reader",
    (await page.locator(".topic-chip .topic-x").first().getAttribute("aria-label"))
  ...
  check("phone width constrains the stage",
    (await page.locator("#preview-stage").evaluate((el) => el.style.maxWidth)) === "390px");
  ...
  check("with the site's default image when there's no photo",
    (await page.locator(".share-image img").getAttribute("src")).includes("/og/"));
  check("and it reports how the lengths compare",
    (await page.locator(".share-notes").textContent()).includes("of about 60 characters"));

  await page.fill("#f-title", "x".repeat(80));
  await page.waitForTimeout(400);
  check("an over-long headline is flagged",
    (await page.locator(".share-notes").textContent()).includes("will be cut around 60"));
...

{
  ...
  check("and says how many people are waiting",
    (await page.locator("#btn-desk").textContent()).includes("(3)"),
    await page.locator("#btn-desk").textContent());
  check("which is the React desk, not the old page",
    (await page.locator("#btn-desk").getAttribute("href")).startsWith("/desk/"));
  ...
  check("the bar now says what is being edited",
    (await page.locator("#now-line").textContent()).includes("/insights/"),
