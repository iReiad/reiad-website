...
export interface Lesson {
  ...
  icon?: string;
  status?: Status;
  /** How much this one matters, 1 to 5, drawn as stars on the
      card and on the lesson.

      A ladder of eighty equally weighted rungs is a ladder
      nobody can budget three evenings against, so this is a
      route through rather than a grade. The scale is written out
      in `next/components/lesson/stars.tsx` and must stay one
      scale: 5 is "you cannot invest without this" and 1 is
      "worth knowing, not worth waiting for". */
  stars?: 1 | 2 | 3 | 4 | 5;
  /** Lesson ids this one builds on, which is what makes the
      ladder a graph a check can walk rather than an order
      somebody remembered. `check-money.ts` fails on a `needs`
      that points forward, at nothing, or at a later stage. */
  needs?: string[];
}
...
const STARTER_WHY: Omit<Lesson, "status">[] = [
  {
    slug: "why-invest",
...
const MONEY_STAGES: Stage[] = [
  {
    ...
    blurb:
      "প্রথমে কেন, কীসের জন্য আর কতটা ঝুঁকি। তারপর আট ধাপে বাংলাদেশে বিনিয়োগ শুরু করার পুরো পথ, কোন কাজটা আপনার আর কোনটা অন্যরা করে দেবে।",
    needs: [],
    status: "live",
    sections: [
      {
        ...
        bn: "কেন, কীসের জন্য",
        en: "Why, and what for",
        lessons: STARTER_WHY.map((s) => ({ ...s, status: "live" as Status })),
      },
      {
        id: "steps",
        bn: "আটটা ধাপ",
        en: "The eight steps",
        lessons: STARTER_STEPS.map((s) => ({ ...s, status: "live" as Status })),
      },
    ],
  },

  {
    ...
    blurb:
      "বাজারের ছাব্বিশটা শব্দ, প্রতিটার আলাদা লেখা, বন্ধুকে বোঝানোর ভঙ্গিতে, হিসাব করে দেখার সুযোগসহ। একটার ভেতরে আরেকটার লিংক।",
    needs: ["start"],
    status: "live",
    sections: [
      {
...

  {
    ...
    blurb:
      "বাংলাদেশে তথ্য কোথায় পাওয়া যায়, রিপোর্ট কীভাবে পড়তে হয়, দাম বসানোর পদ্ধতি, আর একটা কোম্পানি নিয়ে সৎভাবে সিদ্ধান্তে আসার পুরো প্রক্রিয়া।",
    needs: ["basics-2"],
    status: "live",
    sections: [
      {
