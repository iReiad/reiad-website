...
/** Live, or promised and not yet written. A "soon" stage is
    visible and browsable and its unwritten lessons say আসছে, so
    this is never a reason to leave a rung out of the ladder. */
export type Status = "live" | "soon";

/** One page. `slug` is the file name without `.html` and is half
    of the id progress is filed under, so renaming one loses
    whoever had ticked it. */
export interface Lesson {
  ...
  blurb: string;
  minutes: number;
  /** Starter steps only, and drawn as a badge on the card. */
  risk?: "low" | "mid" | "high";
  /** A key in `aab/money/icons.js`. */
  icon?: string;
  status?: Status;
}

/** A heading inside a stage. Never a page of its own. */
...
export interface Stage {
  ...
  /** Where this stage's pages go, when they do not go under the
      school's own folder. See LESSON URLs above. */
  base?: string;
  sections: Section[];
}

/** A subject area with its own front page and mount point. */
...
export const schoolFor = (path: string = "/money/"): School =>
...
export const stageLessons = (stage: Stage): LadderLesson[] =>
  stage.sections.flatMap((section) =>
    section.lessons.map((lesson) => ({
      ...lesson,
      stage,
...
export const stageCount = (stage: Stage): { total: number; live: number } => {
...
export const stageMinutes = (stage: Stage): number =>
  stageLessons(stage).reduce((sum, l) => sum + (l.minutes ?? 0), 0);

/** Find a stage by slug. */
export const findStage = (slug: string): Stage | undefined => STAGES.find((s) => s.slug === slug);

/** Find a lesson (and its stage) from a URL path. */
export const findByPath = (path: string): LadderLesson | undefined =>
