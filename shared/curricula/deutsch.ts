...
export interface Teil {
  ...
  bn: string;
  en: string;
  /** The German heading, because this is a German course. */
  de: string;
  /** A key in `aab/deutsch/icons.js`. */
  icon: string;
  minutes: number;
  blurb: string;
  status?: Status;
...
/** A heading inside a Stufe. Never a page. */
export interface StufeSection {
  id: string;
  bn: string;
  en: string;
  de: string;
...
export interface Stufe {
  ...
  who: string;
  blurb: string;
  /** What you will be able to DO at the end of it: the only
      promise a language course should make. */
  can: string;
  status: Status;
  workbook?: Workbook;
  uebung?: string;
  sections: StufeSection[];
}
...
/** A Teil with the rung it hangs off attached, as every caller
    that walks the ladder wants it. */
export interface LadderTeil extends Teil {
  stufe: Stufe;
  section: StufeSection;
  id: string;
  url: string;
  status: Status;
}
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const STUFE_1_SECTIONS: StufeSection[] = [
  {
    id: "anfangen",
    bn: "শুরুর আগে",
    en: "Before you begin",
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const STUFE_2_SECTIONS: StufeSection[] = [
  {
    id: "wen",
    bn: "কে কাকে কী",
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const STUFE_3_SECTIONS: StufeSection[] = [
  {
    id: "erzaehlen",
    bn: "রঙিন গল্প",
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const STUFE_4_SECTIONS: StufeSection[] = [
  {
    id: "zeit",
    bn: "সময়ের সূক্ষ্মতা",
...
export const teilId = (stufe: Stufe, teil: Teil): string => `${stufe.slug}/${teil.slug}`;

/** One day of a Stufe's workbook, as a progress id. */
export const dayId = (stufe: Stufe, n: number): string => `${stufe.slug}/tag-${n}`;

/** Teile of one Stufe, flattened, in order. */
export const stufeTeile = (stufe: Stufe): LadderTeil[] =>
...
export const allTeile = (): LadderTeil[] => STUFEN.flatMap(stufeTeile);

/** How many Teile a Stufe has, and how many are written. */
export const stufeCount = (stufe: Stufe): { total: number; live: number } => {
...
export const stufeMinutes = (stufe: Stufe): number =>
  stufeTeile(stufe).reduce((sum, t) => sum + (t.minutes ?? 0), 0);

/** Find a Stufe by slug. */
export const findStufe = (slug: string): Stufe | undefined =>
  STUFEN.find((s) => s.slug === slug);

/** Find a Teil (and its Stufe) from a URL path. */
export const findByPath = (path: string): LadderTeil | undefined =>
