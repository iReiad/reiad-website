/* ============================================================
   scripts/snapshot.test.mjs: the R2 half of the backup.
...
const d1 = (missing = new Set()) => ({
  prepare(sql) {
    ...
    return {
      bind: () => ({
        all: async () => {
          if (missing.has(table)) throw new Error("no such table");
          let rows = TABLES[table] ?? [];
          if (/WHERE status = 'live'/.test(sql)) {
            rows = rows.filter((a) => a.status === "live");
          }
...
/* ---------- 1. the public backup, again, through the real function ---------- */
{
  ...
  check("live only", backup.articles.map((a) => a.slug), ["live-one"]);
...
/* ---------- 2. the private snapshot ---------- */
{
  ...
  okay("drafts ARE here, unlike the public one",
    snap.tables.articles.some((a) => a.status === "draft"));
...
/* ---------- 3. a table that does not exist yet ---------- */
{
  ...
  okay("articles still there", snap.tables.articles?.length === 2);
}
...
/* ---------- 4. writing it to R2 ---------- */
{
  ...
  const env = {
    MEDIA: {
      put: async (key, body, meta) => { put.push({ key, bytes: body.length, meta }); },
  ...
  okay("it says how big it was", report.bytes > 0);
